class GrepFoxSearch {
  ...
  setupMessageListener() {
    browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
      switch (message.action) {
        case 'search':
          this.performSearch(message.query, message.isRegex);
  ...
  performSearch(query, isRegex = false) {
    this.clearHighlights();
    ...
    try {
      if (isRegex) {
        this.searchWithRegex(query);
      } else {
        this.searchPlainText(query);
      }
  ...
  searchWithRegex(query) {
    ...
    const regex = new RegExp(jsRegex, 'gi');
    this.findAndHighlightMatches(regex);
  }
  ...
  setupFloatingWindowEvents() {
    ...
    searchInput.addEventListener('input', (e) => {
      ...
      searchTimeout = setTimeout(() => {
        this.performSearch(e.target.value, regexCheckbox.checked);
    ...
    regexCheckbox.addEventListener('change', () => {
      if (searchInput.value.trim()) {
        this.performSearch(searchInput.value, regexCheckbox.checked);
        this.updateFloatingUI();
      }
    });
